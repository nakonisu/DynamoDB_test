            ...
            if s3_key.startswith("local/"):
                ...
                if (
                    ...
                    if os.path.exists(expected_path):
                        ...
                        print(
                            f"警報音 {sound_id} のパスを修正しました: {expected_path}"
                        )
...
def devices_page():
    ...
    return render_template(
        "devices.html", devices=devices, alarm_sounds=alarm_sounds
    )
...
def add_alarm_sound():
    ...
    try:
        ...
        if file and allowed_file(file.filename):
            ...
            flash(
                f"警報音「{sound_name}」を登録しました（ID: {sound_id}）",
                "success",
            )
        else:
            flash(
                "許可されていないファイル形式です（wav, mp3, ogg, m4a のみ）",
                "error",
            )
    ...
    try:
        ...
        for i, sound_id in enumerate(sound_ids):
            if sound_id:  # 空でない場合のみ追加
                threshold = {
                    "auto": Decimal(
                        str(generate_random_threshold())
                    )  # ランダム自動閾値
                }

                # 手動閾値が設定されている場合は追加
                if i < len(manual_thresholds) and manual_thresholds[i]:
                    threshold["manual"] = Decimal(
                        str(float(manual_thresholds[i]))
                    )

                device_configs.append(
                    {"sound_id": sound_id, "threshold": threshold}
                )
...
def edit_device_page(device_id):
    ...
    try:
        ...
        # デバイス情報をDecimal対応で変換
        device = decimal_to_float(device)

        return render_template(
            "edit_device.html", device=device, alarm_sounds=alarm_sounds
        )
    except Exception as e:
        flash(f"デバイス編集ページの表示に失敗しました: {str(e)}", "error")
        return redirect(url_for("devices_page"))
    ...
    try:
        ...
        if not local_path:
            ...
            if s3_key.startswith("local/"):
                ...
                return (
                    jsonify({"error": "音声ファイルのパス情報がありません"}),
                    404,
                )
        ...
        if not os.path.exists(local_path):
            return (
                jsonify(
                    {"error": f"音声ファイルが見つかりません: {local_path}"}
                ),
                404,
            )
        ...
        return (
            jsonify({"error": f"音声ファイルの取得に失敗しました: {str(e)}"}),
            500,
        )
